...
def train(args, train_loader, model, criterion, optimizer, epoch):
    ...
    for iteration, batch in enumerate(train_loader, 0):
        ...
        print('=====> epoch[%d/%d] iter: (%d/%d) \tcur_lr: %.10f loss: %.6f time:%.2f' % (epoch + 1, args.max_epochs,
    ...
    print("time_taken_epoch:", time_taken_epoch//60,"min",time_taken_epoch%60)
    print("Remaining training time = %d hour %d minutes %d seconds" % (h, m, s))

    average_epoch_loss_train = sum(epoch_loss) / len(epoch_loss)
    print("Avgloss1:",sum(epoch_loss1) / len(epoch_loss1))
    print("Avgloss2:",sum(epoch_loss2) / len(epoch_loss2))
...
def train_model(args):
    ...
    model = build_model(args.model, n_classes=args.classes)
    ...
    print('save_epoch = 20, val_epoch = 20')
    for epoch in range(start_epoch, args.max_epochs):
        ...
        # validation
        if (epoch+1) % 20 == 0 or epoch == (args.max_epochs - 1):
            epoches.append(epoch)
            mIOU_val, per_class_iu = val(args, valLoader, model)
            mIOU_val_list.append(mIOU_val)
            # record train information
            logger.write("\n%d\t\t%.4f\t\t%.4f\t\t%.7f" % (epoch, lossTr, mIOU_val, lr))
            logger.write("{}".format(per_class_iu))
            logger.flush()
            print("Epoch : " + str(epoch) + ' Details')
            print("Epoch No.: %d\tTrain Loss = %.7f\t mIOU(val) = %.4f\t lr= %.10f\n" % (epoch,
                                                                                        lossTr,
                                                                                        mIOU_val, lr))
            print(per_class_iu)
        else:
            # record train information
            logger.write("\n%d\t\t%.7f\t\t\t\t%.10f" % (epoch, lossTr, lr))
            logger.flush()
            print("Epoch : " + str(epoch) + ' Details')
            print("Epoch No.: %d\tTrain Loss = %.7f\t lr= %.10f\n" % (epoch, lossTr, lr))
        ...
        if epoch >= args.max_epochs - 10:
            ...
            temp_model_file_name = args.savedir + '/model_' + 'temp' + '.pth'
            torch.save(state, temp_model_file_name)
